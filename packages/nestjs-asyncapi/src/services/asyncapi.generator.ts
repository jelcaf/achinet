...
export class AsyncapiGenerator {
  ...
  public async generate(contract: AsyncApiDocument): Promise<string> {
    ...
    const tmpDir = path.join(os.tmpdir(), `asyncapi-${Date.now()}-${Math.random().toString(36).slice(2)}`);

    const generator: GeneratorOptions = new Generator('@asyncapi/html-template', tmpDir, {
      forceWrite: true,
      templateParams: {
        singleFile: true,
